...
describe('useFigletSync', () => {
  ...
  it('should use custom font option', () => {
    ...
    expect(figlet.textSync).toHaveBeenCalledWith('Test', expect.objectContaining({ font: 'Big' }));
  });
  ...
  it('should calculate responsive width', () => {
    ...
    expect(result.current).toBe('ASCII_ART_SYNC:Test');
    expect(figlet.textSync).toHaveBeenCalledWith('Test', expect.objectContaining({ width: 90 }));
  });
...
describe('Figlet component', () => {
  ...
  it('should pass layout options to useFiglet', async () => {
    render(
      <Figlet text="Test" horizontalLayout="full" verticalLayout="full" whitespaceBreak={false} />
    );
